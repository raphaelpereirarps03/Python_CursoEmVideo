...
def resumo(v=0, porcentDiminuir=0, porcentAumentar=0, formatadoParam=False):
    """
    Método de exibição formatada e tabulada com informações detalhadas e específicas
    :param v: valor recebido / preço
    :param porcentDiminuir: Porcentagem que se deseja subtrair do valor
    :param porcentAumentar: Porcentagem que se deseja somar ao preço
    :param formatadoParam: Se os valores exibidos serão formatados de forma monetária ou não
    :return:
    """
    print('-' * 30)
    ...
    print(f"Dobro do preço: \t{dobro(v, formatadoParam)}")
    print(f"Metade do Preço: \t{metade(v, formatadoParam)}")
    print(f"{porcentDiminuir}% de Redução: \t{diminuir(v, porcentDiminuir, formatadoParam)}")
    print(f"{porcentAumentar}% de Aumento: \t{aumentar(v, porcentAumentar, formatadoParam)}")
    print('-' * 30)
